...
class VertexBufferObject { // everything you need to convert eigen matrix to a video
public:
	...
	void update(const Eigen::MatrixXf& M); // this is the interesting one...

	// Select this VBO for subsequent draw calls
	void bind(); // does nothing but bind, from now on, use this buffer

	// Release the id
	void free(); // simply deallocates the data
};
...
public:
	...
	GLuint program_shader; // pointer to the program
