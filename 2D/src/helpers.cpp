...
void VertexBufferObject::update(const Eigen::MatrixXf& M) {
	assert(id != 0);
	glBindBuffer(GL_ARRAY_BUFFER, id);
    // measures the float val for your current architecture, and 
	glBufferData(GL_ARRAY_BUFFER, sizeof(float)*M.size(), M.data(), GL_DYNAMIC_DRAW);
    // ^  starts at m.data, copies bytes to the gpu
    // this ONLY works with an EIGEN array.
	rows = M.rows();
	cols = M.cols();
	check_gl_error();
}
	...
	const std::string &fragment_data_name)
{
	...
	if (status != GL_TRUE) {
		...
		cerr << "Linker error: " << endl << buffer << endl; // very useful for debugging, writes out error 
		program_shader = 0;
		return false;
	}
